			...
			<div className="flex flex-col lg:flex-row">
				{member.media?.imagesUrl?.[0] && (
					<div className="lg:w-64 lg:flex-shrink-0 relative">
						<div className="relative w-full h-48 sm:h-56 md:h-64 lg:h-full">
							<img
						...
						<div className="lg:hidden absolute bottom-0 left-0 right-0 h-12 bg-gradient-to-t from-black/30 to-transparent" />
					</div>
				)}
				...
				<div className="flex-1 p-4 sm:p-5 md:p-6 lg:p-8">
					<div className="mb-4 md:mb-6">
						<div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-3 mb-3">
							...
							<div className="self-start sm:self-auto">
								<UiBadge 
									size="md" 
									className={`${getRoleColor(member.role)} font-semibold text-xs sm:text-sm`}
								>
					...
					<div className="space-y-3 sm:space-y-4">
						{member.jobTitle && (
							<div className="flex items-start gap-3 p-3 sm:p-4 bg-blue-50 rounded-xl border border-blue-100">
								<div className="w-7 h-7 sm:w-8 sm:h-8 bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5">
									<Badge className="w-3 h-3 sm:w-4 sm:h-4 text-blue-600" />
								</div>
								<div className="flex-1 min-w-0">
									<p className="text-xs sm:text-sm font-medium text-blue-900 mb-1">Должность</p>
						...
						{member.organization && (
							<div className="flex items-start gap-3 p-3 sm:p-4 bg-green-50 rounded-xl border border-green-100">
								...
								<div className="flex-1 min-w-0">
									<p className="text-xs sm:text-sm font-medium text-green-900 mb-1">Организация</p>
									<p className="text-base sm:text-lg font-semibold text-gray-900 break-words">
										{member.organization}
									</p>
								</div>
						...
						{(member.media?.imagesUrl?.length > 0 || member.media?.videoUrl?.length > 0) && (
							<div className="flex flex-wrap gap-1 sm:gap-2 pt-3 sm:pt-4 border-t border-gray-100">
