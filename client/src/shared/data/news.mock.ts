...
export const testHtmlContent = `<div class="content">


<b>В Перми состоялся запуск нового завода HIPEX по производству труб из сшитого полиэтилена PEX-a — аналога REHAU. </b>Проект реализован строительной группой «Развитие» при поддержке Фонда развития промышленности РФ.<br>
<br>
В реализации проекта значимую роль сыграла лидер <b><a target="_blank" href="https://deloros-perm.ru/committees/investments/">Комитета по финансам и инвестициям</a></b>, член регионального совета «Деловой России», генеральный директор ЦФК «Золотые двери» <b><a target="_blank" href="https://deloros-perm.ru/organization/team/members/natalya-saranskaya-/">Наталья Саранская</a></b>. Команда компании ЦФК «Золотые двери» помогла привлечь льготный <b>займ ФРП в размере 600 млн рублей под 1% годовых</b> для закупки современного оборудования.<br>
<br>
<i><b>«Именно в такие моменты понимаешь, что причастен к чему-то большому и значимому. Что твоя компетенция действительно важна и нужна»</b></i>, — отметила Наталья Саранская.<br>
<br>
Новая производственная линия HIPEX собрана по немецким технологиям и оснащена собственной лабораторией комплексных испытаний. Процесс производства труб включает гомогенизацию компонентов в расплаве экструдера и последующую обработку инфракрасным излучением для достижения высокой степени сшивки. Линия позволяет выпускать трубы и фитинги для отопления, теплых полов, вентиляции и водоснабжения — полностью соответствующие современным мировым стандартам.<br>
<br>
Особая разработка завода — фитинги «HI FIT» и «HI STRONG», изготавливаемые методом литья под давлением на термопластавтоматах с использованием материала RADEL PPSU. Продукция проходит строгий контроль на каждом этапе — от входного сырья до гидростатических испытаний и проверки термостойкости.&nbsp;После выхода на проектную мощность предприятие сможет полностью обеспечивать российский рынок современными материалами для систем отопления и водоснабжения, снижая зависимость от импорта.<br>
<br>
Запуск завода стал примером того, как компетенции и инициативы делороссов помогают воплощать масштабные промышленные проекты, укрепляя экономику региона.</div>`;

export const newsMock: INews[] = [
	{
		_id: '0',
		slug: 'perm-ukreplyaet-prom',
		createdAt: new Date('2025-10-15T00:00:00Z'),
		media: {
			imagesUrl: [
				`${API_MOCK.domain}/upload/iblock/2cc/bpni3bi334zizo2ouxarzn9vrt7t1anp/879894.png`,
			],
			videoUrl: [],
		},
		title:
			'В Перми укрепляют промышленный сектор: запущено производство труб HIPEX при поддержке ФРП',
		tags: [
			'Инвестиции',
			'Инвестицииврегион',
			'Комитетпофинансамиинвестициям',
			'СаранскаяНаталья',
		],
